...
static enum tagtype tagtype( const unsigned char *data, id3_length_t length )
{
	if (length >= 3 &&
...
int RageSoundReader_MP3::fill_buffer()
{
	...
	const bool bWasAtEOF = feof(m_pFile);

	size_t rc = fread(mad->inbuf + inbytes, 1, sizeof(mad->inbuf)-inbytes-MAD_BUFFER_GUARD, m_pFile);
	//if( rc < 0 )
	//{
	//	return -1;
	//}

	if ( feof(m_pFile) && !bWasAtEOF )
	{
...
void RageSoundReader_MP3::synth_output()
{
	...
	for(int i=0; i < mad->Synth.pcm.length; i++)
	{
		...
		{
			int16_t Sample = scale(mad->Synth.pcm.samples[chan][i]);
...
{
	if( fseek(m_pFile, byte, SEEK_SET) )
	{
		return 0;
	}
...
int RageSoundReader_MP3::Open(FILE *pFile)
{
	m_pFile = pFile;

	mad->filesize = fsize(m_pFile);
...
{
	return new RageSoundReader_MP3();
}
